...
public class CustomerBasics {
    ...
    public boolean isUniqName(Customer customer) {
        ...
        try {
            pst = con.prepareStatement("select * from cliente where ClienteNombre=? and ClienteContNo=?");
    ...
    public boolean isUpdate(Customer customer) {
        ...
        try {
            pst = con.prepareStatement("select * from cliente where ClienteId=? and ClienteNombre=? and ClienteContNo=? and ClienteDireccion=?");
    ...
    private boolean isSame(Customer customer){
        ...
        try {
            pst = con.prepareStatement("select * from cliente where ClienteId=? and ClienteNombre=? and ClienteContNo=?");
            pst.setString(1,customer.id);
            pst.setString(2,customer.customerName);
            pst.setString(3,customer.customerConNo);
            rs = pst.executeQuery();
